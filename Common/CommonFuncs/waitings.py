...
class Waiting:
    ...
    def explicit_wait(self, locator_type='css', locator_text='', timeout=10, element: WebElement = None, messaging=True):
        try:
            if element:
    ...
    def wait_until_invisibility_of_element(self, locator_type='css', locator='', timeout=5, poll_frequency=0.5,
        ...
        try:
            ...
            with allure.step(f"ERROR: Element: {locator} not invisible on the web page"):
                pass
        if messaging:
            logger.info(f"Element: {locator} invisible on the web page")
    ...
    def wait_until_text_to_be_present_in_element(self, text, locator_text, locator_type='css', timeout=5,
                                                 poll_frequency=0.5, is_input=False, messaging=True):
        element = None
        try:
            locator_type = self.get_by_type(locator_type)
            wait = WebDriverWait(self.browser, timeout=timeout,
    ...
    def wait_until_url_changes(self, expected_url, timeout=5, poll_frequency=0.5, messaging=True):
        try:
