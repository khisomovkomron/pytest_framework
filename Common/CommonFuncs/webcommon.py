...
class WebDriverFactory:
    ...
    def open(self, page, with_token=True):
        base_url = API_HOSTS[self.stand]
        ...
        try:
            if self.stand == 'dev_magnus':
                self.browser.get(f'{base_url}{page}')

                token = TOKENS[self.stand]['admin_nsi']
            elif self.stand == 'dev_bristol':
                self.browser.get(f'{base_url}{page}')

                token = TOKENS[self.stand]['admin_nsi']
            elif self.stand == 'prod_bristol':
                self.browser.get(f'{base_url}{page}')
                token = TOKENS[self.stand]['admin_nsi']

            if with_token:
                self.browser.execute_script(f"window.localStorage.setItem('token', '{token}');")
                self.browser.get(f'{base_url}{page}')
    ...
    def element_presence_check(self, locator, locator_type='css'):
        Waiting(self.browser).wait_until_visibility_of_element(locator, locator_type, messaging=False)
        element_list = self.get_elements(locator, locator_type)
        if len(element_list) > 0:
            logger.info(f"Element present with locator: {locator} and locator_type {locator_type}")
            return True
        else:
            logger.info(f"Element not present with locator: {locator} and locator_type {locator_type}")
            return False
